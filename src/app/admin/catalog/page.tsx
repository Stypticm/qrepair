                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full border
                              ${product.status === 'available' ? 'bg-green-50 text-green-700 border-green-100' :
                                product.status === 'draft' ? 'bg-gray-100 text-gray-600 border-gray-200' :
                                product.status === 'sold' ? 'bg-purple-50 text-purple-700 border-purple-100' :
        ...
        {productToEdit && (
          <div className="fixed inset-0 z-[100] flex justify-end">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg h-full bg-white shadow-2xl flex flex-col pt-12"
            >
              <button 
                onClick={() => setProductToEdit(null)}
                className="absolute top-6 left-6 w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 hover:text-gray-900 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="flex-1 overflow-y-auto px-8 py-6">
                <div className="mb-8">
                  <h2 className="text-2xl font-black text-gray-900 tracking-tight">Редактирование</h2>
                  <p className="text-gray-500 text-sm font-medium">ID: {productToEdit.id}</p>
                </div>

                <form id="edit-form" onSubmit={handleUpdate} className="space-y-6">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Название лота</label>
                    <Input 
                      value={productToEdit.title}
                      onChange={e => setProductToEdit({ ...productToEdit, title: e.target.value })}
                      className="rounded-2xl border-gray-100 bg-gray-50/50 h-14 text-base font-bold focus:ring-blue-500/10 focus:border-blue-500 transition-all"
                      required
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Бренд</label>
                      <Input 
                        value={productToEdit.brand || ''}
                        onChange={e => setProductToEdit({ ...productToEdit, brand: e.target.value })}
                        className="rounded-2xl border-gray-100 bg-gray-50/50 h-12 text-sm font-bold"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Модель</label>
                      <Input 
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Текущий статус</label>
                  ...
                  <div className="space-y-4 p-5 bg-blue-50/30 rounded-3xl border border-blue-100/50">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-gray-500 font-medium">Будет предлагаться к основным товарам</p>
                      </div>
                      <input
                        type="checkbox"
                    ...
                    {productToEdit.isAccessory && (
                      ...
                      >
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Для бренда</label>
                            <Input 
                              value={productToEdit.targetBrand || ''}
                              onChange={e => setProductToEdit({ ...productToEdit, targetBrand: e.target.value })}
                              className="rounded-xl border-gray-200 bg-white h-10 text-xs font-bold"
                              placeholder="Apple"
                            />
                          </div>
                          <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Для модели</label>
                            <Input 
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Описание (Markdown)</label>
                    <textarea 
                      value={productToEdit.description || ''}
                      onChange={e => setProductToEdit({ ...productToEdit, description: e.target.value })}
                      className="w-full rounded-2xl border-gray-100 bg-gray-50/50 p-4 text-sm font-medium outline-none focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all min-h-[120px]"
              ...
              <div className="p-8 bg-gray-50 border-t border-gray-100 flex gap-4">
                <Button 
                  ...
                  variant="outline" 
                  onClick={() => setProductToEdit(null)}
                  className="flex-1 rounded-2xl h-14 font-bold border-gray-200 hover:bg-white transition-all shadow-sm"
                >
                  Отмена
                </Button>
                <Button 
      ...
      <Dialog open={!!productToDelete} onOpenChange={() => setProductToDelete(null)}>
        <DialogContent className="rounded-[2.5rem] max-w-sm w-[95vw] border-none shadow-2xl p-8">
          <DialogHeader>
            ...
            <DialogTitle className="text-2xl font-black text-gray-900 tracking-tight">Удалить товар?</DialogTitle>
            <DialogDescription className="pt-2 text-gray-500 font-medium leading-relaxed">
          ...
          <DialogFooter className="flex flex-col gap-3 pt-8 sm:justify-start">
            ...
            <Button
              ...
              className="w-full rounded-2xl h-14 font-bold border-gray-100 hover:bg-gray-50 transition-all"
            >
              Отмена
            </Button>
          </DialogFooter>

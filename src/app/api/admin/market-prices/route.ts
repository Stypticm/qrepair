...
export async function GET(request: NextRequest) {
  try {
    ...
    // Группируем по устройствам для удобства
    const groupedPrices = marketPrices.reduce(
      (acc: any, price) => {
        ...
        acc[deviceKey].minPrice = Math.min(
          acc[deviceKey].minPrice,
          price.price
        )
        acc[deviceKey].maxPrice = Math.max(
          acc[deviceKey].maxPrice,
          price.price
        )
    ...
    Object.values(groupedPrices).forEach((group: any) => {
      const totalPrice = group.prices.reduce(
        (sum: number, p: any) => sum + p.price,
        0
      )
      group.averagePrice = Math.round(
        totalPrice / group.prices.length
      )
    ...
    return NextResponse.json({
      ...
      count: Object.keys(groupedPrices).length,
    })
  } catch (error) {

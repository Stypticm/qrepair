...
export default function OptimizedPhoneSelector({ open: externalOpen, onOpenChange: setExternalOpen }: OptimizedPhoneSelectorProps) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-[420px] p-0 rounded-[40px] overflow-hidden border-none bg-white shadow-2xl">
        <DialogHeader className="px-8 pt-8 pb-4">
          ...
          <DialogTitle className="text-2xl font-black tracking-tight text-center text-gray-900">
            {step === 'category' ? 'Оценка устройства' :
              step === 'summary' ? 'Проверьте данные' :
                `Шаг ${STEPS.indexOf(step)} из 9`}
          </DialogTitle>
        </DialogHeader>
